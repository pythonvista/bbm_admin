<template>
  <div>
    <div v-if="loader">
      <UtilsSkeleton1 :numbers="10"></UtilsSkeleton1>
    </div>

    <div v-else>
      <div class="q-pa-md">
        <q-table
          title="All Data Plans"
          :rows="rows"
          :columns="columns"
          row-key="name"
          @row-click="SelectRow"
        ...
        <q-card-section class="q-pt-none">
          <form method="POST" class="w-full">
            <p class="text-lg w-full text-center mb-5 font-bold">
              {{ dform.Network }} {{ dform.Size }} {{ dform.Validity }}
            </p>
            <div class="space-y-4">
              <div>
                <label
              ...
              <div>
                <div class="flex items-center justify-between">
                  ...
                  <span class="text-red">Dont remove the naira symbol!</span>
                </div>
                <div class="mt-2.5">
                  <input
                    type="text"
              ...
              <div>
                ...
                <div class="mt-2.5">
                  <q-select
                    ...
                    :options="['CORPORATE GIFTING', 'GIFTING', 'SME']"
                    placeholder="Plan Size"
                    class="block w-full text-gray-900 placeholder-gray-600 bg-white rounded-mdfocus:border-gray-900 focus:ring-gray-900 caret-gray-900"
                  />
                </div>
              </div>
              <div>
    ...
    async GetNetworks() {
      try {
        ...
        const res = await axios.get(
          `/api/admin/services?id=${userCred.uid}&service=DATAPACKS`,
          {
            headers: {
    ...
    async SaveChanges() {
      try {
        ...
        const res = await axios.post(
          `/api/admin/services?id=${userCred.uid}&services=DATAPACKS`,
          this.dform,
          {
            headers: {
              Accept: "application/json",

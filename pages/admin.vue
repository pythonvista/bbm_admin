<template>
  ...
  <section v-else class="relative min-h-screen py-12 bg-gray-900 sm:py-16 lg:py-20">
...
export default {
    ...
    methods: {
        async Login() {
            try {
                this.loading = true
                await auth.login(this.email, this.passwd)
                ShowSnack('Logged In Successful', 'positive')
                this.$router.push({ path: '/dashboard' })
                this.loading = false
            } catch (err) {
                this.loading = false
                console.log(err)
                ShowSnack(err.code || 'Error Occured Logging In', 'negative')
    ...
    mounted() {
        const userCred = TokenGetter()
        if (userCred.uid && userCred.token) {
            this.$router.push({ path: "/dashboard" })
        } else {
            this.spinner = false
        }
